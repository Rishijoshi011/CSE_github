...
int main() {
    ...
    for (int i = 0; i < str.length(); i++) {
        cout << "bit "<< assci_val[i] << " xor by " << var_xor  << ": "<< (assci_val[i] ^ var_xor) << endl; // ! XOR
        cout << "bit "<< assci_val[i] << " AND by " << var_and  << ": "<< (assci_val[i] & var_and) << endl; // ! AND
        cout << "bit "<< assci_val[i] << " OR by " << var_or  << ": "<< (assci_val[i] | var_or) << endl; // ! OR
        cout << "bit "<< assci_val[i] << " left shift by " << 1  << ": "<< (assci_val[i] << 1) << endl; // ! left shift
        cout << "bit "<< assci_val[i] << " right shift by " << 1  << ": "<< (assci_val[i] >> 1) << endl; // ! right shift
        cout << endl;
    }
